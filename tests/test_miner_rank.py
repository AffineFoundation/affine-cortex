...
def test_rank_table_renders_old_single_table_shape_with_sampling_marks():
    ...
    assert "org/champion              | Qwen3-32B" in out
    assert "org/challenger            | Qwen3.6-35B" in out
    assert "org/q                     | -" in out
...
def test_model_type_label_prefers_model_type_without_repo_fallback():
    assert _model_type_label({"model_type": "qwen3"}) == "Qwen3-32B"
    assert _model_type_label({"model_type": "qwen3_5_moe"}) == "Qwen3.6-35B"
    assert _model_type_label({"model_type": "custom_arch"}) == "custom_arch"
    assert _model_type_label({"model": "Qwen/Qwen3-30B-A22B"}) == "-"
    assert _model_type_label({}) == "-"
