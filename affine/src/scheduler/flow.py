...
class FlowScheduler:
    ...
    async def _read_or_create_behavior_gate_snapshot(
        ...
        absolute deadline first, a read-only ``row=None`` snapshot would make
        the expiry CAS impossible because there is no deadline to compare in
        DynamoDB.  Create the row with the record's original deadline and
        strongly re-read it before making any expiry decision.
        """
