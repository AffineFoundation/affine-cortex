...
# Promote cadence — hardcoded constants (the queue-window refactor stripped
# the per-env ``rotation_interval`` / ``rotation_count`` knobs that used to
# drive this from SystemConfig). The mover is teacher-specific bookkeeping,
# not part of the per-env competition config.
MOVER_PROMOTE_INTERVAL_SEC = int(os.getenv("MOVER_PROMOTE_INTERVAL_SEC", "300"))
MOVER_PROMOTE_COUNT = int(os.getenv("MOVER_PROMOTE_COUNT", "5"))
